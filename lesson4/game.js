...
async function saveProgressToGoogleSheets(action = 'save', earnedExp = 0) {
    try {
        ...
        // Отправляем на сервер с общим опытом
        const dataToSend = {
            action: 'save',
            password: 'teacher123',
            ...
            earnedExp: earnedExp,              // Опыт за уровень (без вопроса)
            totalExperience: totalExperience,  // Общий опыт (включая вопрос)
            lessonNumber: 4,
            partNumber: 0,
...
function handleAnswer(selectedIndex, correctIndex) {
    ...
    if (selectedIndex === correctIndex) {
        ...
        if (questionAttempts === 1) {
            ...
            setTimeout(async () => {
                await saveProgressToGoogleSheets('update', 1);
                console.log('Опыт за вопрос сохранен');
            }, 100);
        ...
        if (questionAttempts < 3) {
            ...
            setTimeout(async () => {
                await saveProgressToGoogleSheets('update', -1);
                console.log('Отрицательный опыт сохранен');
            }, 100);
...
// Обновить функцию updateExperienceDisplay (примерно строка 120)
function updateExperienceDisplay() {
    ...
    // Обновляем текст с учетом отрицательных значений
    let displayText = `Опыт: ${totalExperience}`;
    if (totalExperience < 0) {
        displayText = `Опыт: ${totalExperience} ❌`;
...
function calculateExperience() {
    ...
    // 🆕 Ключ для завершенных уровней ДЛЯ ЭТОГО УЧЕНИКА (как в уроке 2)
    const partKey = '4.0';
...
window.hideIntroAndStart = async function() {
    ...
    // 🆕 Инициализируем данные о пройденных уровнях для этого ученика
...
function showWinModal(isPartComplete = false) {
    // 🆕 ДОБАВЬ ЭТУ СТРОКУ:
    const earnedExpForLevel = calculateExperience();
